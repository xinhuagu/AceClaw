...
public final class UdsListener {
    ...
    private static final String SOCKET_FILE_NAME = "aceclaw.sock";
    private static final Path DEFAULT_DIR = Path.of(System.getProperty("user.home"), ".aceclaw");
    ...
    public void start() throws IOException {
        ...
        acceptThread = Thread.ofVirtual().name("aceclaw-uds-accept").start(this::acceptLoop);
    ...
    private void acceptLoop() {
        while (running.get()) {
            try {
                ...
                Thread.ofVirtual().name("aceclaw-uds-conn").start(() -> handleConnection(client));
            } catch (IOException e) {
                if (running.get()) {
